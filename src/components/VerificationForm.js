...
function VerificationForm(props) {
  ...
  return (
    <>
          <Snackbar anchorOrigin={{vertical: 'top',horizontal: 'center'}} open={open} autoHideDuration={3000}><SnackbarContent style={{
  ...
  /></Snackbar>
   ...
   <div className='CloseButton1'>
        ...
        <div className='register-modal'>
          ...
          <span className="info">Please enter the information correctly as it is going to be controlled</span>
          <label className='labels'>Phone</label>
          <TextField type="tel" id="phone" name="phone" placeholder="5325323232" required error={phoneError} helperText={phoneErrorText} className={classes.root} InputProps={{className: classes.root}} pattern="[0-9]{3}-[0-9]{2}-[0-9]{3}" onChange={(e)=> setPhone(e.target.value)}/>
          ...
          <TextField type="date" name="dob" required error={dobError} helperText={dobErrorText} className={classes.root} InputProps={{className: classes.root}} onChange={(e)=> setDOB(e.target.value.split("-").reverse().join("-"))}/>
          <div>
            <button
